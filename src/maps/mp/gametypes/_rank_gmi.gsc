...
InitializeBattleRank()
{
	...
	game["br_ammo_gunclips_2"] = 4;
	game["br_ammo_gunclips_3"] = 4;
	game["br_ammo_gunclips_4"] = 4;
...
GetGunAmmo(weapon)
{
	...
	switch(weapon)
	{
		...
		//Give garand more ammo (updated Feb 2021)
		case "m1garand_mp":

		return (clip_count + 11) * clip_size;
...
artillery_strike_sounds()
{
	...
	while (1)
	{
		...
		// Modified for DOM mod - Don't play arty sounds if all-cap
		if (isdefined(level.allcap_respawn_art_bypass) && level.allcap_respawn_art_bypass)
		{
			wait 1;
			continue;
		}
